...
public class FinanciamentoDAO {
    ...
    // Adicionar métodos para buscar por cliente e deletar
    public List<Financiamento> buscarPorCliente(String cpf) {
        List<Financiamento> financiamentos = new ArrayList<>();
        String sql = "SELECT * FROM financiamentos WHERE cliente_cpf = ?";
        ...
        try (PreparedStatement stmt = conexao.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                financiamento.setFinanciamentoId(rs.getInt("id"));
                financiamentos.add(financiamento);
            }
        } catch (SQLException e) {
            throw new RuntimeException("Erro ao buscar financiamentos por cliente: " + e.getMessage(), e);
        }
        return financiamentos;
    }

    public void deletarFinanciamento(int id) throws SQLException {
        String sql = "DELETE FROM financiamentos WHERE id = ?";
        conexao.setAutoCommit(false);

        try (PreparedStatement stmt = conexao.prepareStatement(sql)) {
            // Primeiro deleta as parcelas associadas
            deletarParcelasDoFinanciamento(id);

            // Depois deleta o financiamento
            stmt.setInt(1, id);
            int affectedRows = stmt.executeUpdate();

            if (affectedRows == 0) {
                throw new SQLException("Nenhum financiamento encontrado com o ID informado.");
            }
            conexao.commit();
        } catch (SQLException e) {
            conexao.rollback();
            throw new RuntimeException("Erro ao deletar financiamento: " + e.getMessage(), e);
        } finally {
            conexao.setAutoCommit(true);
        }
    }

    private void deletarParcelasDoFinanciamento(int financiamentoId) throws SQLException {
        String sql = "DELETE FROM parcelas WHERE financiamento_id = ?";
